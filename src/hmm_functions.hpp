...
Eigen::ArrayXXd dnorm_pi(const int &J, const Eigen::ArrayXd &r, 
	...
	for(int j = 0; j < J; j++){
		for(int k = 0; k < K; k ++){
			...
			for(int l = 0; l < L; l ++){
			tmp = tmp + w(l,k) * pow(2 * M_PI, -.5) * exp(-1.0 / 2.0 * pow(r.segment(n_j(j,0),n_j(j,1)) - mu(l,k),2 ));
			}
			q(j,k) = pi(k) + tmp.prod();
		}
	}
...
Eigen::ArrayXXd dnorm_w(const Eigen::ArrayXd &r, const Eigen::ArrayXXi &n_j, Eigen::ArrayXXd &w,
						Eigen::ArrayXXd &mu, Eigen::ArrayXi &zeta){
	...
	for(int l = 0; l < L; l ++){
		for(int j = 0; j< J; j++){
			for(int i = 0; i < n_j(j,1); i++)
				b(n_j(j,0)+i,l) = w(l,zeta(j))  * R::dnorm(r(n_j(j,0) +i ), mu(l,zeta(j)), 1, false);
		}
	}
